...
const PartnerLayout: React.FC = () => {
    ...
    return (
        <div className={`flex min-h-screen w-full flex-col max-w-md mx-auto shadow-2xl overflow-hidden relative transition-colors duration-300 ${
            isDark ? 'bg-[#121014] text-white' : 'bg-[#FDFCF8] text-[#121014]'
        }`}>
            ...
            {/* Bottom Navbar */}
            <nav className={`fixed bottom-0 left-0 right-0 h-[74px] px-2 py-3 flex justify-between items-center z-50 max-w-md mx-auto border-t overflow-x-auto no-scrollbar ${
                isDark 
                ...
                <LayoutGroup id="partner-navbar">
                    {navItems.map(({ to, label, Icon }) => (
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                `relative flex flex-col items-center gap-1 transition-colors px-1 py-1 rounded-xl overflow-visible min-w-[48px] flex-1 ${
                                    isActive 
                                        ? 'text-primary' 
                                        : isDark 
