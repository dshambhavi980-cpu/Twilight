...
const SongLyrics: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchSession = async () => {
            ...
            if (data) {
                ...
                setLoading(false);
                sendGameNotification(couple, user.id, 'Finish the Lyrics', '/games/song-lyrics', 'invite');
            }
        };

        fetchSession();

        const ch = supabase.channel(`game_sl_${couple.id}`)
            ...
            (payload) => {
                ...
                if (newSess.game_type === 'song_lyrics') setSession(newSess);
            })
            .subscribe();
    ...
    if (loading || !session) return <div className="flex h-screen items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-pink-500"></div></div>;
                            ...
                            <div className={`p-6 rounded-2xl mb-6 ${isDark ? 'bg-white/5' : 'bg-white shadow-lg'}`}>
                                {Object.entries(board_state.scores).map(([playerId, score]) => (
                                    <div key={playerId} className="flex items-center justify-between py-3 border-b border-white/10 last:border-0">
                                        ...
                                        <span className="text-2xl font-bold text-pink-400">{score} / {TOTAL_ROUNDS}</span>
                                    </div>
                                ))}
                            </div>
