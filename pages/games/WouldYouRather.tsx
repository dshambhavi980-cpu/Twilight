...
interface WYRState {
    phase: 'pick_category' | 'playing' | 'revealed' | 'finished';
    category: string;
    rounds: WYRRound[];
    currentRound: number;
    totalRounds: number;
...
const emptyState = (): WYRState => ({
    phase: 'pick_category', category: '', rounds: [], currentRound: 0,
    totalRounds: 10, matchCount: 0, usedIds: [],
});
...
const WouldYouRather: React.FC = () => {
        ...
        (async () => {
            ...
            try {
                const { data: existing } = await supabase.from('game_sessions').select('*')
                    .eq('couple_id', couple.id).eq('game_type', 'would_you_rather')
    ...
    const broadcast = useCallback((updated: GameSession) => {
        ...
        supabase.from('game_sessions').update({ board_state: updated.board_state, status: updated.status }).eq('id', updated.id);
    }, []);
            ...
            <main className="flex-1 flex flex-col items-center px-5 gap-4 pb-24 pt-12">
                ...
                {game.status === 'waiting' && (
                    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} className="flex flex-col items-center gap-3 mt-12">
                        ...
                        <p className="text-sm text-gray-400">Send partner to Games → Would You Rather</p>
                    </motion.div>
                )}
