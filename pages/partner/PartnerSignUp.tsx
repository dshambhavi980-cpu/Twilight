...
const PartnerSignUp: React.FC = () => {
    ...
    try {
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            full_name: fullName,
            is_partner: true, // Explicitly set partner flag
          },
        },
      });
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.session) {
        // Force redirect to partner join page
        navigate('/partner');
      }
    } catch (err: any) {
      setError(err.message);
  ...
  if (verifying) {
    return (
      <div className="min-h-screen flex flex-col bg-background-dark text-white font-display">
         <div className="flex items-center p-4 pb-2 z-10">
            <button onClick={() => setVerifying(false)} className="text-white flex size-12 items-center justify-center hover:bg-white/5 rounded-full">
               <span className="material-symbols-outlined">arrow_back_ios_new</span>
            </button>
            <h2 className="text-lg font-bold flex-1 text-center pr-12">Verify Partner Account</h2>
         </div>
         <div className="flex-1 flex flex-col items-center justify-center p-6">
            <div className="text-center mb-8">
               <span className="material-symbols-outlined text-5xl text-blue-400 mb-4">mark_email_read</span>
               <h1 className="text-2xl font-bold mb-2">Check your email</h1>
               <p className="text-white/60">We sent a code to <span className="text-white">{email}</span></p>
            </div>
            <form onSubmit={handleVerifyOtp} className="w-full max-w-sm space-y-6">
               <input
                 value={otp}
                 onChange={(e) => setOtp(e.target.value)}
                 className="w-full h-14 bg-white/5 border border-white/10 rounded-xl px-5 text-center text-xl tracking-widest focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all"
                 placeholder="000000"
                 maxLength={8}
               />
               <button type="submit" disabled={loading} className="w-full h-14 bg-blue-500 hover:bg-blue-600 rounded-xl font-bold text-white transition-all shadow-lg shadow-blue-500/20">
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background-dark text-white font-display">
      <div className="flex items-center p-4 pb-2 justify-between z-10">
        ...
        <h2 className="text-lg font-bold flex-1 text-center pr-12">Partner Sign Up</h2>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6">
        <div className="text-center mb-8">
          <div className="w-20 h-20 bg-blue-500/20 rounded-full flex items-center justify-center mx-auto mb-6 border border-blue-500/30">
             <span className="material-symbols-outlined text-4xl text-blue-400">volunteer_activism</span>
          </div>
          <h1 className="text-3xl font-bold mb-2">Join as Supporter</h1>
          <p className="text-white/60 text-sm">Create an account to connect with<br/>your partner and support their journey.</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="w-full max-w-md space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input name="fullName" required className="w-full h-14 bg-white/5 border border-white/10 rounded-xl px-5 focus:border-blue-500 outline-none transition-all" placeholder="Enter your name" />
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input name="password" type={showPassword ? "text" : "password"} required minLength={8} className="w-full h-14 bg-white/5 border border-white/10 rounded-xl px-5 pr-12 focus:border-blue-500 outline-none transition-all" placeholder="••••••••" />
          ...
          <button type="submit" disabled={loading} className="w-full h-14 bg-blue-500 hover:bg-blue-600 rounded-xl font-bold text-white transition-all shadow-lg shadow-blue-500/20 mt-6 flex items-center justify-center gap-2">
            <span>{loading ? 'Creating Account...' : 'Create Partner Account'}</span>
        ...
        <p className="mt-8 text-white/40 text-sm">
          Already have an account? <button onClick={() => navigate('/partner/login')} className="text-blue-400 hover:text-blue-300 font-semibold">Log In</button>
        </p>
      </div>
    </div>
  );
